<template>
  <div class="search">
    ...
    <section class="list" v-if="!searchResultFlag" ref="searchList">
      <ul class="list_container">
        <router-link :to="{path:'/shop', query:{id:item.id}}" tag="li" v-for="item in searchShopList" :key="item.id" class="list_li">
          <section class="item_left">
            <img :src="imgBaseUrl + item.image_path" class="restaurant_img">
          </section> <section class="item_right">
  ...
  export default {
    ...
    methods: {
      search() {
        ...
        if (keyword) {
          this.$store.dispatch('searchShopList', keyword);
        }
      }
    },
    computed: {
